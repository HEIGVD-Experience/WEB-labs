...
export class Shape {
  ...
  static getRandomShapeType() {
    return Math.floor(Math.random() * shapeTypes.length);
  }
  ...
  getCoordinates(rotation = this.rotation) {
    const rotationNumber = shapeTypes[this.shapeType].length;
    return shapeTypes[this.shapeType][rotation % rotationNumber];
  }
}
