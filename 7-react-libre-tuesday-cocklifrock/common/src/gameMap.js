export class GameMap {
    ...
    dropShape(shape) {
        if (!this.testShape(shape)) {
            console.log("Shape is conflicting with something while trying to drop; aborting!")
            return
        }
    ...
    getBlocksPerPlayer() {
        ...
        for (let row = 0; row < this.height; row++) {
            for (let col = 0; col < this.width; col++) {
                const playerId = this.getPlayerAt(row, col);

                if (playerId !== -1) {
                    const playerCount = playerMap.get(playerId) || 0;
                    playerMap.set(playerId, playerCount + 1);
                }
            }
        }
