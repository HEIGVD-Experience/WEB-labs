...
export class Renderer {
    ...
    renderBlock(col, row, color) {
        const x = cellToPixel(col);
        const y = cellToPixel(row);
    
        // Ajout de l'ombre
        this.context.shadowColor = 'rgba(0, 0, 0, 0.5)';
        this.context.shadowBlur = 20;
        this.intensity = this.playerId === this.game.currentPlayerId ? 1 : 0.5;
        this.context.shadowOffsetX = this.intensity * 2;
        this.context.shadowOffsetY = this.intensity * 2;
    
        // Dessin du bloc
        this.context.fillStyle = color;
        this.context.fillRect(x, y, cellPixelSize, cellPixelSize);
        this.context.strokeRect(x, y, cellPixelSize, cellPixelSize);
    
        // Réinitialisation de l'ombre pour d'autres dessins
