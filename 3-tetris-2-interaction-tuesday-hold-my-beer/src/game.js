...
export class Game extends Map {
  ...
  moveShape(id, col) {
    ...
    if (player === undefined) {
      console.log("Cannot find player " + id + "; ignoring");
      return;
    }
    ...
    if (shape === undefined) {
      console.log(
        "Shape " + id + " does not exist; cannot move it; ignoring"
      );
      return;
    }
  ...
  rotateShape(id, rotation) {
    ...
    if(rotation === "right") {
      let new_rotation = (shape.rotation + 1) % shapeTypes[shape.shapeType].length;
      if(this.map.testShape(shape, shape.row, shape.col, new_rotation)) {
        shape.rotation = new_rotation;
      }
    } else {
      let new_rotation = (shape.rotation - 1 + shapeTypes[shape.shapeType].length) % shapeTypes[shape.shapeType].length;
      if(this.map.testShape(shape, shape.row, shape.col, new_rotation)) {
        shape.rotation = new_rotation;
      }
    }
  ...
  onMessage(playerId, message) {
    switch(message.constructor) {
      ...
      case DropMessage:
        ...
        break;
      default:
        console.log("ERROR : Received message from player " + playerId + ": " + message.constructor.name);
    }
  }
}
